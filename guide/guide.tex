...
{\em \emph{flowPsi} } is an open source CFD solver that is developed
...
It uses finite volume approximation of the Euler and Navier Stokes equation with 
upwind, second and fourth order scheme and Crank-Nicholson time stepping scheme. 
It enables to sovler problem on stationary, moving rigid and deforming meshes with 
overset mesh capabilites.
...
So far, the only gust implemented in \emph{flowPsi} are \emph{1-cos} 
and \emph{step} gust.
...
\item \textbf{pulse} gust

\begin{verbatim}
gustSpecification: < type=pulse, Usg=3 m/s, 
                     gvel=[102.078794566 m/s, 0, 0], 
                     gnorm=[0., 1., 0.], Ogust=[-1., 0., 0.] >
\end{verbatim}
The parameters in this set are
\begin{itemize}
 \item type specifies type of gust. For \emph{pulse} the type is \textbf{pulse}
 \item Usg specifies maximum vertical gust velocity
 \item gnorm specifies vector normal to gust plane (if gust in is x-z plane, specify [0,1,0])
 \item Ogust origin of gust 
\end{itemize}
...
through a cosimulation engine. The cosimulation engine is the \emph{lsipdx} utility\cite{Jirasek_2014}
