...
\item {\tt timeStepReset }

This variable resets time to zero when restarting from unsteady solution and {\tt timeStepMode}
set to {\tt unsteady}. The values are either {\tt no} or {\tt yes}. The default value is {\tt no}.
...
\subsection{Note on gust specification}
When restarting the analysis with gust, be careful about seting the intial time. If the time is not 0, the 
initial position of the gust will not be as specified by {\tt Ogust}, the gust will be moved
according to {\tt Ogust + time * gvel} position. The easiest way is to reset the time upon restart to 
initial time by setting parameter {\tt timeStepReset: yes}.
