...
namespace gridMotion {
  ...
  using std::max ;
//--------------------------------------------------------------------  
inline real frsum_basic(real x[3],real beta[9],real xc[3], int a) {
  real rr1 = sqrt((x[0]-beta[0])*(x[0]-beta[0])+
                    (x[1]-beta[1])*(x[1]-beta[1])+
                    (x[2]-beta[2])*(x[2]-beta[2])) ;
  real rr2 = sqrt((x[0]-beta[3])*(x[0]-beta[3])+
                    (x[1]-beta[4])*(x[1]-beta[4])+
                    (x[2]-beta[5])*(x[2]-beta[5])) ;
  real rr3 = sqrt((x[0]-beta[6])*(x[0]-beta[6])+
                    (x[1]-beta[7])*(x[1]-beta[7])+
                    (x[2]-beta[8])*(x[2]-beta[8])) ;
  real x40 = 4.*xc[0]-beta[0]-beta[3]-beta[6] ;
  real x41 = 4.*xc[1]-beta[1]-beta[4]-beta[7] ;
  real x42 = 4.*xc[2]-beta[2]-beta[5]-beta[8] ;
  
  real rr4 = sqrt((x[0]-x40)*(x[0]-x40)+
                    (x[1]-x41)*(x[1]-x41)+
                    (x[2]-x42)*(x[2]-x42)) ;
  return 0.25*(pow(rr1,-a)+pow(rr2,-a)+ pow(rr3,-a)+pow(rr4,-a)) ;
}

  inline real frsum(real x[3],real beta[9],real xc[3],real J[9],int a) {
    real rr1 = sqrt((x[0]-beta[0])*(x[0]-beta[0])+
		     (x[1]-beta[1])*(x[1]-beta[1])+
		     (x[2]-beta[2])*(x[2]-beta[2])) ;
    real rr2 = sqrt((x[0]-beta[3])*(x[0]-beta[3])+
    ...
    real J4 = .25*pow(rr4,-a-2) ;
    real J1 = .25*pow(rr1,-a-2) ;
    J[0] = -real(a)*((beta[0]-x[0])*J1+(x[0]-x40)*J4) ;
    J[1] = -real(a)*((beta[1]-x[1])*J1+(x[1]-x41)*J4) ;
    J[2] = -real(a)*((beta[2]-x[2])*J1+(x[2]-x42)*J4) ;
    real J2 = .25*pow(rr2,-a-2) ;
    J[3] = -real(a)*((beta[3]-x[0])*J2+(x[0]-x40)*J4) ;
    J[4] = -real(a)*((beta[4]-x[1])*J2+(x[1]-x41)*J4) ;
    J[5] = -real(a)*((beta[5]-x[2])*J2+(x[2]-x42)*J4) ;
    real J3 = .25*pow(rr3,-a-2) ;
    J[6] = -real(a)*((beta[6]-x[0])*J3+(x[0]-x40)*J4) ;
    J[7] = -real(a)*((beta[7]-x[1])*J3+(x[1]-x41)*J4) ;
    J[8] = -real(a)*((beta[8]-x[2])*J3+(x[2]-x42)*J4) ;
  ...
  void get_fexact(const vector<nodeDataBase> &nodeDataCollection,
                  ...
                  real radius,
                  vector3d<real> xc,
                  const real testpnts[][3],
                  real fexact[],
                  int npnts) {
  
    for(int j=0;j<npnts;++j) {
      vector3d<real> isopt(testpnts[j][0],testpnts[j][1],testpnts[j][2]) ;
      vector3d<real> pt = vector3d<real>(xc) + real(radius)*isopt ;
      real f = 0 ;
      for(int i=start;i<end;++i) {
	const vector3d<real> npos   = nodeDataCollection[i].pos;
	const real r = norm(pt-npos) ;
	const real wn = nodeDataCollection[i].weight;
	f += wn * pow(r,-exponent) ;
      }
      fexact[j] = f ;
    }
...
#pragma ivdep
  ...
  void makeQuad(const vector<nodeDataBase> &nodeDataCollection,
	...
	vector3d<realF> isopt(testpnts[jj][0],testpnts[jj][1],testpnts[jj][2]) ;
	vector3d<realF> pt = xc + radius*isopt ;
	real rr1 = norm(qp1-pt) ;
	real rr2 = norm(qp2-pt) ;
	real rr3 = norm(qp3-pt) ;
	real rr4 = norm(4.*xc-qp1-qp2-qp3-pt) ;
	int a = exponent ;
	real ya=0.25*(pow(rr1,-a)+pow(rr2,-a)+ pow(rr3,-a)+pow(rr4,-a)) ;
	real r = fexact[jj]-ya ;
    ...
    real XC[3] = {xc.x,xc.y,xc.z} ;
    vector3d<real> xcd(xc.x,xc.y,xc.z) ;
    real beta[9] ;
    // initial beta guess
	...
	B[i] = 0 ;
      ...
      real mxr = 0 ;
      for(int jj=0;jj<nfit;++jj) {
	vector3d<real> isopt(testpnts[jj][0],testpnts[jj][1],testpnts[jj][2]) ;
	vector3d<real> pt = xcd + real(radius)*isopt ;
	real x[3] = {pt.x,pt.y,pt.z} ;
	real ya = frsum(x,beta,XC,J,exponent) ;
	real r = (fexact[jj]-ya) ;
	mxr = max(mxr,fabs(r)) ;
	for(int i=0;i<9;++i) {
	  B[i] += J[i]*r ;
	...
	for(int jj=0;jj<nfit;++jj) {
	  ...
	  vector3d<real> pt = xcd + real(radius)*isopt ;
	  real x[3] = {pt.x,pt.y,pt.z} ;
	  real ya = frsum_basic(x,btmp,XC,exponent) ;
	  real r = (fexact[jj]-ya) ;
	  mxrstep = max(mxrstep,fabs(r)) ;
	}
	if(mxrstep > mxr)
	  alpha = alpha/real(2+ls) ;
	else {
	  break ;
	}
      }
    ...
    qp1 = vector3d<realF>(beta[0],beta[1],beta[2]) ;
    qp2 = vector3d<realF>(beta[3],beta[4],beta[5]) ;
    qp3 = vector3d<realF>(beta[6],beta[7],beta[8]) ;
  ...
  int buildTreeRecurse(vector<treeInfoBase> &nodeDataTree,
    ...
    real xmin = nodeDataSort[start].pos.x ;
    real xmax = xmin ;
    real ymin = nodeDataSort[start].pos.y ;
    real ymax = ymin ;
    real zmin = nodeDataSort[start].pos.z ;
    real zmax = zmin ;

    for(int i=start+1;i<end;++i) {
      vector3d<real> p = nodeDataSort[i].pos ;
      xmin = min(xmin,p.x) ;
      xmax = max(xmax,p.x) ;
      ymin = min(ymin,p.y) ;
      ymax = max(ymax,p.y) ;
  ...
  int buildTreeRecurseProc(vector<treeInfoBase> &nodeDataTree,
    ...
    real fac = real(psize/2)/real(psize) ;

    if((xmax-xmin > ymax-ymin) && (xmax-xmin > zmax-zmin)) {
      // split in x coordinate                  
        
      //Set pval to the center of the bounding box x-coordinates
      realF pval = xmin + fac*(xmax-xmin) ;
      //    cout << "xpval = " << pval << ' ' << end-start << endl ;
    ...
    int psplit = int(realToDouble(real(psize)*real(split-start)/real(end-start)+.5))+proclims[node].first ;
  ...
  void computeQuadPointsTree(const vector<treeInfoBase> &nodeDataTree,
			     ...
			     vector<realF> &radius,
			     vector<realF> &err,
			     vector<Array<vector3d<realF>,3> > &q,
			     int node) {
    const real factor = 3 ;
    ...
    if(left >= 0) { // follow branches
      ...
      realF r2 = max<realF>(1e-15f,dot(centroid[node],centroid[node])*1e-10f) ;
      for(int i=start;i<end;++i) {
	vector3d<realF> dv = nodeDataSort[i].pos-centroid[node] ;
	r2 = max(r2,dot(dv,dv)) ;
      }
      radius[node] = sqrt(r2) ;
      real ftest[ntest] ;
      real rtest = radius[node]*factor ;
      get_fexact(nodeDataSort,start,end,exponent,rtest,centroid[node],
		 testpnts,&ftest[0],ntest) ;
      real rw = 1./weights[node] ;
	...
	ftest[i] *= rw ;
      ...
      real weight = 0 ;
      vector3d<real> xsum(0,0,0) ;
      for(int i=start;i<end;++i) {
	real w = nodeDataSort[i].weight ;
	weight += w ;
	xsum += w*vector3d<real>(nodeDataSort[i].pos) ;
      }
      weights[node] = weight ;
      xsum *= (1./weight) ;
      centroid[node] = xsum ;
      real r2 = max<real>(1e-15,dot(xsum,xsum)*1e-10) ;
      for(int i=start;i<end;++i) {
	vector3d<realF> dv = nodeDataSort[i].pos-vector3d<realF>(xsum) ;
	r2 = max(r2,real(dot(dv,dv))) ;
      }
      radius[node] = sqrt(r2) ;
      real ftest[ntest] ;
      real rtest = radius[node]*factor ;
  ...
  computeDeformationQuadsTree(const vector<real> &weights, int node) {
    const int left = nodeDataTree[node].left ;
    const int right = nodeDataTree[node].right ;
    if(left >= 0) { // follow branches
    ...
    const int ntst = 4 ;
    vector3d<realF> xc = centroid[node] ;
    vector3d<realF> rtestpts[ntst] ;
    ...
    for(int i=0;i<ntst;++i) {
      rtestpts[i] *= 1./max<realF>(norm(rtestpts[i]),1e-10f) ;
      if(dot(rtestpts[i],rtestpts[i]) < 0.99)
        rtestpts[i] = vector3d<realF>(1.,0,0) ;
    }
    
    // extract displacement and rotation matrix at centroid and 
    // compute centroid values for deformation
    vector<vector<vector3d<real> > > defs(4) ;
    for(int i=0;i<4;++i) {
      vector<vector3d<real> > tmp(ntst) ;
      defs[i].swap(tmp) ;
      for(int k=0;k<ntst;++k)
	defs[i][k] = vector3d<real>(0,0,0) ;
    }

    vector3d<real> meanvals[4] ;
    for(int i=0;i<4;++i) 
      meanvals[i] = vector3d<real>(0,0,0) ;

    const int exponent = 3 ;
    const realF factor = 3.0 ;
    real wexactsum[ntst] ;
    for(int i=0;i<ntst;++i)
      wexactsum[i] = 0 ;
	...
	vector3d<realF> dside = xc-centroid[side] ;
	tensor3d<realF> rside = qrot[side][0];
	rside += qrot[side][1] ;
	rside += qrot[side][2] ;
	rside += qrot[side][3] ;
	rside *= 0.25 ;
	vector3d<realF> disp = 0.25*(qdisp[side][0]+qdisp[side][1]+
				     qdisp[side][2]+qdisp[side][3]) +
	  dot(rside,dside) ;
	meanvals[0] += vector3d<real>(weights[side]*disp) ;
	meanvals[1] += vector3d<real>(weights[side]*rside.x) ;
	meanvals[2] += vector3d<real>(weights[side]*rside.y) ;
	meanvals[3] += vector3d<real>(weights[side]*rside.z) ;
      }
      for(int i=0;i<4;++i) {
	...
	rotmat.y.y -= 1.0 ;
	rotmat.z.z -= 1.0 ;
	vector3d<real> dp(xc-nodeDataSort[i].pos) ;
	vect3d displT = vector3d<real>(nodeDisp[i])+dot(rotmat,dp) ;
	real w = nodeDataSort[i].weight ;
	meanvals[0] += w*displT ;
	...
	vector3d<realF> rtest = rtestpts[j]*radius[node]*factor+xc ;
	const real r = norm(nodeDataSort[i].pos-rtest) ;
	const real wn = nodeDataSort[i].weight ;
	const real wr = wn*pow(r,-exponent) ;
	wexactsum[j] += wr ;
	defs[0][j] += wr*displT ;
    ...
    for(int s=0;s<nrsample;++s) {
      // test point
      vector3d<realF> pt = rtestpts[s]*radius[node]*factor+xc ;
	...
	W[s][i] = 0 ;
      real Wsum = 0 ;
      for(int i=0;i<3;++i) {
	const real r = norm(pt-qpts[node][i]) ;
	...
	Wsum += W[s][i] ;
      ...
      for(int i=0;i<4;++i)
	W[s][i] *= rWsum ;
      real J[3] = {W[s][0]-W[s][3],W[s][1]-W[s][3],W[s][2]-W[s][3]} ;
    ...
    real det = (+A00*A11*A22+A01*A12*A02+A02*A01*A12
		  -A02*A11*A02-A00*A12*A12-A01*A01*A22) ;
    real ATAinv[3][3] ;
    real rdet = 1./(det+((det>0.0)?1e-12:-1e-12)) ;
    ...
    for(int field=0;field<4;++field) { 
      ...
      vector3d<real> Y[3] = {zero,zero,zero} ;
      for(int s=0;s<nrsample;++s) {
	real J[3] = {W[s][0]-W[s][3],W[s][1]-W[s][3],W[s][2]-W[s][3]} ;
	...
	for(int i=0;i<3;++i) {
      ...
      vector3d<real> v1= meanvals[field] ;
      vector3d<real> v2 = v1 ;
      vector3d<real> v3 = v1 ;
      vector3d<real> v4 = v1 ;
	...
	// test point
	vector3d<realF> pt = xc + rtestpts[s]*radius[node]*factor ;
	// Weights associated with quadripole point for this test point
	real W[4] = {0,0,0,0} ;
	real Wsum = 0 ;
	for(int i=0;i<3;++i) {
	...
	const real r3 = norm(pt-4.*xc+qpts[node][0]+qpts[node][1]+qpts[node][2]) ;
	W[3] = pow(r3,-exponent) ;
	Wsum += W[3] ;
	real rWsum = 1./Wsum ;
	for(int i=0;i<4;++i)
	...
	qdisp[node][2] = v3 ;
	qdisp[node][3] = v4 ;
	droterr = max(droterr,realF(err[field]/(max<real>(norm(meanvals[field]),1e-4)))) ;
	break ;
      case 1:
	qrot[node][0].x = v1 ;
	qrot[node][1].x = v2 ;
	qrot[node][2].x = v3 ;
	qrot[node][3].x = v4 ;
	droterr = max(droterr,realF(err[field])) ;
	break ;
      case 2:
	qrot[node][0].y = v1 ;
	qrot[node][1].y = v2 ;
  ...
  void buildWeightsApprox(const vector<nodeDataBase> &nodeDataSort,
    ...
    weights = vector<real>(tsz) ;
    centroid = vector<vector3d<realF> >(tsz) ;
    radius = vector<realF>(tsz) ;
    qpts = vector<Array<vector3d<realF>,3> >(tsz) ;
    qerr = vector<realF>(tsz) ;
    ...
    // Collect Centroids
    if(p > 1) {
      vector<vector3d<realF> > centbuf(treeptn[r+1]-treeptn[r]) ;
	...
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&centbuf[0],centbuf.size()*3,REALF_MPI_TYPE,
		     &centroid[treeptn[0]],
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     comm) ;
      
      centbuf.resize(p) ;
      vector3d<realF> cent = centroid[proctreebase[r]] ;
      MPI_Allgather(&cent,3,REALF_MPI_TYPE,&centbuf[0],3,REALF_MPI_TYPE,comm) ;
      for(int i=0;i<p;++i)
	centroid[proctreebase[i]] = centbuf[i] ;
    }
    if(p > 1) {
      // Collect radius
      vector<realF > buf(treeptn[r+1]-treeptn[r]) ;
      for(int i=treeptn[r];i<treeptn[r+1];++i)
	...
	recv_counts[i] = (treeptn[i+1]-treeptn[i]) ;
      ...
      for(int i=1;i<p;++i)
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size(),REALF_MPI_TYPE,
		     &radius[treeptn[0]],
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     comm) ;
	// collect qerr
	...
	buf[i-treeptn[r]] = qerr[i] ;
      MPI_Allgatherv(&buf[0],buf.size(),REALF_MPI_TYPE,
		     ...
		     comm) ;
      buf.resize(p) ;
      realF rad = radius[proctreebase[r]] ;
      MPI_Allgather(&rad,1,REALF_MPI_TYPE,&buf[0],1,REALF_MPI_TYPE,comm) ;
      for(int i=0;i<p;++i)
	radius[proctreebase[i]] = buf[i] ;
    }
    if(p > 1) {
      // Collect weights
      vector<real > buf(treeptn[r+1]-treeptn[r]) ;
      for(int i=treeptn[r];i<treeptn[r+1];++i)
	...
	recv_counts[i] = (treeptn[i+1]-treeptn[i]) ;
      ...
      for(int i=1;i<p;++i)
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size(),REAL_MPI_TYPE,
		     &weights[treeptn[0]],
		     &recv_counts[0],&displs[0], REAL_MPI_TYPE,
		     comm) ;
      buf.resize(p) ;
      real w = weights[proctreebase[r]] ;
      
      MPI_Allgather(&w,1,REAL_MPI_TYPE,&buf[0],1,REAL_MPI_TYPE,comm) ;
      for(int i=0;i<p;++i)
	weights[proctreebase[i]] = buf[i] ;
    }
    // Collect quad points
    if(p > 1) {
      vector<Array<vector3d<realF>,3> > buf(treeptn[r+1]-treeptn[r]) ;
      for(int i=treeptn[r];i<treeptn[r+1];++i)
	...
	recv_counts[i] = (treeptn[i+1]-treeptn[i])*9 ;
      ...
      for(int i=1;i<p;++i)
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size()*9,REALF_MPI_TYPE,
		     &qpts[treeptn[0]],
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     comm) ;
      buf.resize(p) ;
      Array<vector3d<realF>,3> qp = qpts[proctreebase[r]] ;
      vector<Array<vector3d<realF>,3> > buf2(p) ;
      MPI_Allgather(&qp,9,REALF_MPI_TYPE,&buf2[0],9,REALF_MPI_TYPE,comm) ;
      for(int i=0;i<p;++i)
    ...
    vector<realF> radii2(treework.size(),0) ;
	...
	vector3d<realF> dv = (nodeDataSort[j].pos-centroid[node]) ;
	r2 = max(r2,dot(dv,dv)) ;
      }
      radii2[i] = r2 ;
    }
    vector<realF> radii(treework.size(),0) ;
      
    MPI_Allreduce(&radii2[0],&radii[0],treework.size(),
		  REALF_MPI_TYPE,MPI_MAX,comm) ;
    for(size_t i=0;i<treework.size();++i) {
      int node = treework[i] ;
    ...
    const int exponent = 3 ;
    const realF factor = 3.0 ;
    vector<real> ftesti(ntest*treework.size()) ;
    for(size_t i=0;i<treework.size();++i) {
      int node = treework[i] ;
      int sloc = max(start,nodeDataTree[node].start) ;
      real rtest = radius[node]*factor ;
    ...
    vector<real> ftest(ntest*treework.size()) ;
    MPI_Allreduce(&ftesti[0],&ftest[0],ntest*treework.size(),REAL_MPI_TYPE,
		  MPI_SUM,comm) ;
    for(size_t i=0;i<treework.size();++i) {
      int node = treework[i] ;
      real rtest = radius[node]*factor ;
  ...
  void buildDefApprox(deformApproxTree &approxTree,
    ...
    approxTree.qdisp = vector<Array<vector3d<realF>,4> >(tsz) ;
    approxTree.qrot  = vector<Array<tensor3d<realF>,4> >(tsz) ;
    approxTree.drot  = vector<realF>(tsz) ;
	...
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size(),REALF_MPI_TYPE,
		     ...
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     bcomm) ;
      realF tmp = approxTree.drot[proctreebase[r]] ;
      vector<realF> buf2(pbuild) ;
      MPI_Allgather(&tmp,1,REALF_MPI_TYPE,&buf2[0],1,REALF_MPI_TYPE,bcomm) ;
      for(int i=0;i<pbuild;++i)
	approxTree.drot[proctreebase[i]] = buf2[i] ;
    }
    if(pbuild > 1) {
      vector<Array<vector3d<realF>, 4> > buf(treeptn[r+1]-treeptn[r]) ;
      for(int i=treeptn[r];i<treeptn[r+1];++i)
	...
	recv_counts[i] = (treeptn[i+1]-treeptn[i])*12 ;
      ...
      for(int i=1;i<pbuild;++i)
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size()*12,REALF_MPI_TYPE,
		     &approxTree.qdisp[treeptn[0]],
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     bcomm) ;
      vector<Array<vector3d<realF>, 4> > buf2(pbuild) ;
      Array<vector3d<realF>,4> tmp = approxTree.qdisp[proctreebase[r]] ;
      MPI_Allgather(&tmp,12,REALF_MPI_TYPE,&buf2[0],12,REALF_MPI_TYPE,bcomm) ;
      for(int i=0;i<pbuild;++i) 
	approxTree.qdisp[proctreebase[i]] = buf2[i] ;
    }
    if(pbuild > 1) {
      vector<Array<tensor3d<realF>, 4> > buf(treeptn[r+1]-treeptn[r]) ;
      for(int i=treeptn[r];i<treeptn[r+1];++i)
	...
	recv_counts[i] = (treeptn[i+1]-treeptn[i])*36 ;
      ...
      for(int i=1;i<pbuild;++i)
	displs[i] = displs[i-1]+recv_counts[i-1] ;
      MPI_Allgatherv(&buf[0],buf.size()*36,REALF_MPI_TYPE,
		     &approxTree.qrot[treeptn[0]],
		     &recv_counts[0],&displs[0], REALF_MPI_TYPE,
		     bcomm) ;
      vector<Array<tensor3d<realF>, 4> > buf2(pbuild) ;
      Array<tensor3d<realF>,4> tmp = approxTree.qrot[proctreebase[r]] ;
      MPI_Allgather(&tmp,36,REALF_MPI_TYPE,&buf2[0],36,REALF_MPI_TYPE,bcomm) ;
      for(int i=0;i<pbuild;++i) 
    ...
    vector<Array<vector3d<real>, 4> > meanvals(topsz) ;
    for(int j=0;j<topsz;++j)
      for(int i=0;i<4;++i) 
	meanvals[j][i] = vector3d<real>(0,0,0) ;
    for(int j=0;j<topsz;++j) {
      int node = treework[j] ;
	...
	rotmat.y.y -= 1.0 ;
	rotmat.z.z -= 1.0 ;
	vector3d<real> dp(xc-approxTree.nodeDataSort[i].pos) ;
	vector3d<real> displT = vector3d<real>(approxTree.nodeDisp[i])+dot(rotmat,dp) ;
	real w = approxTree.nodeDataSort[i].weight ;
    ...
    {     
      vector<Array<vector3d<real>, 4> > meanvalsc(topsz) ;

      MPI_Allreduce(&meanvals[0],&meanvalsc[0],topsz*4*3,
		    REAL_MPI_TYPE,MPI_SUM,bcomm) ;
    ...
    const int ntst = 4 ;
    vector<Array<vector3d<realF>,ntst> > rtestpts(topsz) ;
    for(int j=0;j<topsz;++j) {
      int node = treework[j] ;
      vector3d<realF> xc = approxTree.centroid[node] ;
      ...
      for(int i=0;i<ntst;++i)
	rtestpts[j][i] *= 1./max<realF>(norm(rtestpts[j][i]),1e-10f) ;
    }
    vector<Array<Array<vector3d<real>,ntst>,4> > defs(topsz) ;
    vector<Array<real,ntst> > wexactsum(topsz) ;
    for(int j=0;j<topsz;++j) {
      for(int i=0;i<ntst;++i)
	wexactsum[j][i] = 0.0 ;
      for(int i=0;i<ntst;++i)
	for(int k=0;k<4;++k)
	  defs[j][k][i] = vector3d<realF>(0.0,0.0,0.0) ;
	...
	rotmat.z.z -= 1.0 ;
	vector3d<real> dp(xc-approxTree.nodeDataSort[i].pos) ;
	vect3d displT = vector3d<real>(approxTree.nodeDisp[i])+
	  dot(rotmat,dp) ;
      
	for(int t=0;t<ntst;++t) {
	  vector3d<realF> rtest = rtestpts[j][t]*approxTree.radius[node]*factor+xc ;
	  const real r = norm(approxTree.nodeDataSort[i].pos-rtest) ;
	  const real wn = approxTree.nodeDataSort[i].weight ;
	  const real wr = wn*pow(r,-exponent) ;
	  wexactsum[j][t] += wr ;
    ...
    {
      vector<Array<real,ntst> > wexactsumc(topsz) ;
      MPI_Allreduce(&wexactsum[0],&wexactsumc[0],topsz*ntst,
		    REAL_MPI_TYPE,MPI_SUM,bcomm) ;
      wexactsum.swap(wexactsumc) ;
    }
    {
      vector<Array<Array<vector3d<real>,ntst>,4> > defsc(topsz) ;
    ...
    for(int j=0;j<topsz;++j) {
      ...
      for(int s=0;s<nrsample;++s) {
	// test point
	vector3d<realF> pt = rtestpts[j][s]*approxTree.radius[node]*factor+xc ;
	...
	real Wsum = 0 ;
	for(int i=0;i<3;++i) {
	  const real r = norm(pt-approxTree.qpts[node][i]) ;
	  W[s][i] = pow(r,-exponent) ;
	  Wsum += W[s][i] ;
	}
	...
	vector3d<real> v1= meanvals[j][field] ;
	vector3d<real> v2 = v1 ;
	vector3d<real> v3 = v1 ;
	vector3d<real> v4 = v1 ;
	...
	for(int s=0;s<ntst;++s) {
	  // test point
	  vector3d<realF> pt = xc + rtestpts[j][s]*approxTree.radius[node]*factor ;
	  // Weights associated with quadripole point for this test point
	  real W[4] = {0,0,0,0} ;
	  real Wsum = 0 ;
	  for(int i=0;i<3;++i) {
	...
	case 0:
	  ...
	  droterr = max(droterr,realF(err[field]/(max<realF>(norm(meanvals[j][field]),1e-4)))) ;
	  break ;
	case 1:
  ...
  void weightApproxTree::build(const vector<vector3d<realF> > &pnts,
    ...
    if(r<pbuild) {
      if(r == 0 && pbuild != p) {
	int tsz = nodeDataTree.size() ;
	MPI_Send(&cweight[0],tsz,REALF_MPI_TYPE,pbuild,21,comm) ;
	MPI_Send(&centroid[0],tsz*3,REALF_MPI_TYPE,pbuild,22,comm) ;
	MPI_Send(&radius[0],tsz,REALF_MPI_TYPE,pbuild,23,comm) ;
	MPI_Send(&qpts[0],tsz*3*3,REALF_MPI_TYPE,pbuild,24,comm) ;
	MPI_Send(&qerr[0],tsz,REALF_MPI_TYPE,pbuild,25,comm) ;
	int flag = 0 ;
	MPI_Status stat ;
	MPI_Recv(&flag,1,MPI_INT,pbuild,29,comm,&stat) ;
      ...
      if(r == pbuild) {
	MPI_Status stat ;
	MPI_Recv(&cweight[0],tsz,REALF_MPI_TYPE,0,21,comm,&stat) ;
	MPI_Recv(&centroid[0],tsz*3,REALF_MPI_TYPE,0,22,comm,&stat) ;
	MPI_Recv(&radius[0],tsz,REALF_MPI_TYPE,0,23,comm,&stat) ;
	MPI_Recv(&qpts[0],tsz*3*3,REALF_MPI_TYPE,0,24,comm,&stat) ;
	MPI_Recv(&qerr[0],tsz,REALF_MPI_TYPE,0,25,comm,&stat) ;
      }
      MPI_Bcast(&cweight[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&centroid[0],tsz*3,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&radius[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&qpts[0],tsz*3*3,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&qerr[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      if(r == pbuild) {
	int flag = 1 ;
  ...
  WeightApprox(real &weight, real &werr,
               const vector3d<real> &pos, int node,
               int a, int b, real L, real alphaL,
               real errpn) const {
    const vector3d<real> dvn = pos-vector3d<real>(centroid[node]) ;
    const real dn = norm(dvn) ;
    //    if(node == 0) 
    ...
    if(dn > 1.5*radius[node]) {
      ...
      real err = (qerr[node])/(rfac*rfac) ;
      if(rfac < .9) // increase error estimate if we are too close
	err *= 10.0 ;
      // estimated weight
      const vector3d<realF> posf(pos) ;
      const real r1 = 1./norm(posf-qpts[node][0]) ;
      const real r2 = 1./norm(posf-qpts[node][1]) ;
      const real r3 = 1./norm(posf-qpts[node][2]) ;
      //    qp4 = 4.*xc - qp1 -  qp2 - qp3 ;
      const real r4 = 1./norm(posf-4.*centroid[node]+
				qpts[node][0]+qpts[node][1]+qpts[node][2]) ;
      const real w1 = pow(L*r1,a)+pow(alphaL*r1,b) ;
      const real w2 = pow(L*r2,a)+pow(alphaL*r2,b) ;
      const real w3 = pow(L*r3,a)+pow(alphaL*r3,b) ;
      const real w4 = pow(L*r4,a)+pow(alphaL*r4,b) ;
      const real W = 0.25*cweight[node] ;
      const real wn = W*(w1+w2+w3+w4) ;
      err *= wn ;
	...
	real rnrvec = 1./max<real>(norm(rvec),1e-20) ;
	const real w1 = pow(L*rnrvec, a) ;
	const real w2 = pow(alphaL*rnrvec, b) ;
      
	const real w = wn*(w1 + w2);
    ...
    real diff = -nodeDataTree[node].split ;
  ...
  void buildDeformApprox(deformApproxTree &approxTree,
    ...
    approxTree.nodeDisp = vector<vector3d<realF> >(nsz) ;
    ...
    if(r < pbuild) {    
      ...
      approxTree.cweight = vector<realF>(tsz) ;
      for(int i=0;i<tsz;++i) {
    ...
    if(r<pbuild) {
      if(r == 0 && pbuild != p) {
	int tsz = approxTree.nodeDataTree.size() ;
	MPI_Send(&approxTree.cweight[0],tsz,REALF_MPI_TYPE,pbuild,21,comm) ;
	MPI_Send(&approxTree.centroid[0],tsz*3,REALF_MPI_TYPE,pbuild,22,comm) ;
	MPI_Send(&approxTree.radius[0],tsz,REALF_MPI_TYPE,pbuild,23,comm) ;
	MPI_Send(&approxTree.qpts[0],tsz*3*3,REALF_MPI_TYPE,pbuild,24,comm) ;
	MPI_Send(&approxTree.qerr[0],tsz,REALF_MPI_TYPE,pbuild,25,comm) ;
	MPI_Send(&approxTree.qdisp[0],tsz*3*4,REALF_MPI_TYPE,pbuild,26,comm) ;
	MPI_Send(&approxTree.qrot[0],tsz*9*4,REALF_MPI_TYPE,pbuild,27,comm) ;
	MPI_Send(&approxTree.drot[0],tsz,REALF_MPI_TYPE,pbuild,28,comm) ;
	int flag = 0 ;
	MPI_Status stat ;
	MPI_Recv(&flag,1,MPI_INT,pbuild,29,comm,&stat) ;
      }
      ...
      int tsz = approxTree.nodeDataTree.size() ;
      approxTree.cweight = vector<realF>(tsz) ;
      approxTree.centroid = vector<vector3d<realF> >(tsz) ;
      approxTree.radius = vector<realF>(tsz) ;
      approxTree.qpts = vector<Array<vector3d<realF>,3> >(tsz) ;
      approxTree.qerr = vector<realF>(tsz) ;
      approxTree.qdisp = vector<Array<vector3d<realF>,4> >(tsz) ;
      approxTree.qrot  = vector<Array<tensor3d<realF>,4> >(tsz) ;
      approxTree.drot  = vector<realF>(tsz) ;
      if(r == pbuild) {
	MPI_Status stat ;
	MPI_Recv(&approxTree.cweight[0],tsz,REALF_MPI_TYPE,0,21,comm,&stat) ;
	MPI_Recv(&approxTree.centroid[0],tsz*3,REALF_MPI_TYPE,0,22,comm,&stat) ;
	MPI_Recv(&approxTree.radius[0],tsz,REALF_MPI_TYPE,0,23,comm,&stat) ;
	MPI_Recv(&approxTree.qpts[0],tsz*3*3,REALF_MPI_TYPE,0,24,comm,&stat) ;
	MPI_Recv(&approxTree.qerr[0],tsz,REALF_MPI_TYPE,0,25,comm,&stat) ;
	MPI_Recv(&approxTree.qdisp[0],tsz*3*4,REALF_MPI_TYPE,0,26,comm,&stat) ;
	MPI_Recv(&approxTree.qrot[0],tsz*9*4,REALF_MPI_TYPE,0,27,comm,&stat) ;
	MPI_Recv(&approxTree.drot[0],tsz,REALF_MPI_TYPE,0,28,comm,&stat) ;
      }
      MPI_Bcast(&approxTree.cweight[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.centroid[0],tsz*3,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.radius[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.qpts[0],tsz*3*3,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.qerr[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.qdisp[0],tsz*3*4,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.qrot[0],tsz*9*4,REALF_MPI_TYPE,0,bcomm) ;
      MPI_Bcast(&approxTree.drot[0],tsz,REALF_MPI_TYPE,0,bcomm) ;
      if(r == pbuild) {
	int flag = 1 ;
	MPI_Send(&flag,1,MPI_INT,0,29,comm) ;
      }
  ...
  void deformApproxTree::
  TreeApprox(vector3d<real> &disp, real &weight, real &werr,
	     const vector3d<real> &pos, int node,
	     int a, int b, real L, real alphaL,
	     real errpn) const {
    const vector3d<real> dvn = pos-vector3d<real>(centroid[node]) ;
    ...
    if(dn > 1.5*radius[node]) {
      ...
      // Estimate error
      const real rfac = dn/(3.*radius[node]) ;
      //    real err = nodeDataTree[node].err/(rfac*rfac)+nodeDataTree[node].drot/(rfac*rfac) ;
      real err = (qerr[node]+drot[node])/(rfac*rfac) ;
      if(rfac < .9) // increase error estimate if we are too close
	err *= 10.0 ;
      // estimated weight
      const vector3d<realF> posf(pos) ;
      ...
      // Check to see if the error is within the acceptable range
      if((err+werr)/(weight+wn) < errpn) { 
	vector3d<realF> pc = vector3d<realF>(pos)-centroid[node] ;
	//	const vector3d<realF> def = dot(cRotations[node],pc) + cDisp[node]  ;
	const vector3d<realF> disp1 = dot(qrot[node][0],pc)+qdisp[node][0] ;
	const vector3d<realF> disp2 = dot(qrot[node][1],pc)+qdisp[node][1] ;
	const vector3d<realF> disp3 = dot(qrot[node][2],pc)+qdisp[node][2] ;
	const vector3d<realF> disp4 = dot(qrot[node][3],pc)+qdisp[node][3] ;
	const vector3d<realF> dispf = W*(w1*disp1+w2*disp2+w3*disp3+w4*disp4) ;
	weight += wn ;
	werr += err ;
	disp += vector3d<real>(dispf) ;
